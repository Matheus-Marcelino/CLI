...
class ConverterMp3:
    ...
    def convert_mp3(self) -> None:
        """Converte seu mp4 para mp3"""

        if self.__checking():
            return '\033[1;31mExtensão não suportada!\033[m' 

        if '.mp3' not in name_mp4[-4:]:
            name_mp4 = name_mp4 + '.mp3'
        ...
        def converter() -> bool:
            ...
            audio_clip.write_audiofile(name_mp4)
            audio_clip.close()
            video_clip.close()
            return True
        ...
        def move_file() -> None:
            ...
            try:
                rename(name_mp4, f'output/{name_mp4}')
